...
export default function App() {
	...
	const removeState = useCallback(
		(s) => {
			...
			setStartState((prev) =>
				prev === s ? states.filter((x) => x !== s)[0] || "" : prev,
			);
			setTransitions((prev) => {
				const next = { ...prev };
				delete next[s];
	...
	const updateTransition = useCallback(
		(oldFrom, oldSym, oldTo, newFrom, newSym, newTo) => {
			setTransitions((prev) => {
				const next = JSON.parse(JSON.stringify(prev));
				// Remove old
				if (next[oldFrom]?.[oldSym]) {
					next[oldFrom][oldSym] = next[oldFrom][oldSym].filter(
						(t) => t !== oldTo,
					);
				...
				if (!next[newFrom][newSym].includes(newTo))
					next[newFrom][newSym].push(newTo);
				return next;
			});
		},
		[],
	...
	const exportJSON = useCallback(() => {
		...
		Object.values(transitions).forEach((t) =>
			Object.keys(t).forEach((s) => alph.add(s)),
		);
		const data = {
			mode,
			states,
	...
	const importJSON = useCallback(
		(jsonStr) => {
			try {
				...
				setStates(d.states || []);
				setAcceptStates(d.acceptStates || []);
				setStartState(d.startState || "");
